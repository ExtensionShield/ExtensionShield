...
async function main() {
  ...
  for (const route of routes) {
    ...
    try {
      ...
      // Wait until react-helmet-async has written the canonical link (every SEO page sets one).
      await page
        .waitForFunction(() => !!document.querySelector('link[rel="canonical"]'), { timeout: 20000 })
        .catch(() => {});
